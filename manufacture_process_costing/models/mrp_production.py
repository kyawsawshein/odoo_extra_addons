...
class MrpProduction(models.Model):
    ...
    material_cost_ids = fields.One2many(
        "direct.material.cost", "production_material_id", string="Material Costs"
    )
    labour_cost_ids = fields.One2many(
        "direct.labour.cost", "production_labour_id", string="Labour Costs"
    )
    overhead_cost_ids = fields.One2many(
        "direct.overhead.cost", "production_overhead_id", string="Overhead Costs"
    )
    ...
    total_actual_material_cost = fields.Float(
        ...
        compute="_compute_total_actual_material_cost",
        store=True,
    ...
    total_actual_overhead_cost = fields.Float(
        ...
        compute="_compute_total_actual_overhead_cost",
        store=True,
    ...
    def action_cancel_button(self):
        ...
        reason = self.env["mrp.cancel.reason"].create({"manufacturing_id": self.id})
        return {
    ...
    def _compute_total_actual_cost(self):
        ...
        for rec in self:
            rec.total_actual_cost = (
                rec.total_actual_material_cost
